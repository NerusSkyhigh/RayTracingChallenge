...
public:
    StripePattern() :   a( std::make_unique<Monocromatic>(Color::White())),
                        b( std::make_unique<Monocromatic>(Color::Black()) ),
                        transform(Matrix::identity(4)),
                        itransform(Matrix::identity(4)) {}

    StripePattern(const Color& ca, const Color& cb) :   a( std::make_unique<Monocromatic>(ca) ),
                                                        b( std::make_unique<Monocromatic>(cb) ),
                                                        transform(Matrix::identity(4)),
                                                        itransform(Matrix::identity(4)) {}

    StripePattern(std::unique_ptr<Pattern> a, std::unique_ptr<Pattern> b) : a( std::move(a) ),
                                                                            b( std::move(b) ),
                                                                            transform(Matrix::identity(4)),
                                                                            itransform(Matrix::identity(4)){}
    ...
    std::unique_ptr<Pattern> clone() const override {
        auto p = std::make_unique<StripePattern>(
            a->clone(),
            b->clone()
        );
